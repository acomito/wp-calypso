...
export default React.createClass( {
	...
	expiredDomains() {
		...
		if ( expiredDomains.some( domain => domain.isCurrentUserOwner ) ) {
			return this.expiredDomainsOwner( expiredDomains );
		}

		return this.expiredDomainsNonOwner( expiredDomains );
	},

	expiredDomainsOwner( expiredDomains ) {
		let text, renewLink;
		expiredDomains = expiredDomains.filter( domain => domain.isCurrentUserOwner );
	...
	expiredDomainsNonOwner( expiredDomains ) {
		...
		return <Notice status="is-error" showDismiss={ false } key="expired-domains">{ text }</Notice>;
	},
	...
	expiringDomains() {
		...
		if ( expiringDomains.some( domain => domain.isCurrentUserOwner ) ) {
			return this.expiringDomainsOwner( expiringDomains );
		}

		return this.expiringDomainsNonOwner( expiringDomains );
	},

	expiringDomainsOwner( expiringDomains ) {
		let text, renewLink;
		expiringDomains = expiringDomains.filter( domain => domain.isCurrentUserOwner );
	...
	expiringDomainsNonOwner( expiringDomains ) {
		...
		return <Notice status="is-error" showDismiss={ false } key="expiring-domains">{ text }</Notice>;
	},
	...
	unverifiedDomains() {
		...
		if ( domains.some( domain => domain.isCurrentUserOwner ) ) {
			return this.unverifiedDomainNoticeOwner( domains );
		}

		return this.unverifiedDomainNoticeNonOwner( domains );
	},

	unverifiedDomainNoticeOwner( domains ) {
		domains = domains.filter( domain => domain.isCurrentUserOwner );
